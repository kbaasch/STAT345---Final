...
lets_play_idk = function(turns, players) {
  ...
  player_bank_data <- data.frame(matrix(nrow = 2, ncol = players))
    ...
    for (i in 2:turns) {
        ...
        location_data[i,j] <- as.integer(location)
        double3data[i,j] <- lets_move3(location)[[2]]
    ...
    players_with_0 <- vector(length = players)
        
    if (The_Players$Player_Bank[j] <= 0 || "NA" %in% The_Players$Player_Bank[j]) {
      The_Players$Player_Bank[j] == "NA"
      players_with_0[j] <- j
    }  
    
    count_of_false <- sum(which("FALSE" %in% players_with_0))
    if (count_of_false == 1) {
      break
    }
        #length(which())
