...
Chance_Money <- function(Player_Bank){
...
#Need to figure out how to exchange money between players
Community_Chest_Money <- function(Player_Bank){
  Player_Bank = Player_Bank
  draw_card <- sample(1:16, 1, replace = TRUE)
...
```{r}
library(tidyverse)
assigning_properties <- function( ){
  smpl <- sample(1:28, 28, replace = FALSE)
  smpl
  property_order <- data.frame(smpl)
  property_order
}

Owned_Spaces <- sample(1:28, 28, replace = FALSE)
Owned_Spaces
Players <- c("Player 1", "Player 2", "Player 3", "Player 4")
Player_Bank <- c("1250", "1250", "1250", "1250")
...
The_Players_Owned <- data.frame(Players, Owned_Spaces)
The_Players_Owned <- The_Players_Owned %>% pivot_wider(names_from = Players, values_from = Owned_Spaces)
...
Updating_Player_Bank <- function(location, j) {
  if(location == 2 || location == 4 || location == 6 || location == 7 || location == 9 || location == 10 || location == 12 || location == 13 || location == 14 || location == 15 || location == 16 || location == 17 || location == 19 || location == 20 || location == 22 || location == 24 || location == 25 || location == 26 || location == 27 || location == 28 || location == 29 || location == 30 || location == 32 || location == 33 || location == 35 || location == 36 || location == 38 || location == 40){
    ...
    The_Players$Player_Bank[j] <- The_Players$Player_Bank[j] - monopolyBoard$cost[cost]
...
=======
    ...
    return(The_Players)
>>>>>>> 85e132d8f4ea5e8d828e212183ef522bc9720c73
  }
}

Updating_Player_Bank(12)
```
```{r}
