...
lets_play = function(turns, players) {
  ...
  for (k in 2:players) {
  lines(density(location_data[,k]), col = colors[k-1]) # adds a line for each player after the first
  }
  
  # creates density plot for total frequency landing on each space (all players spaces included)
  all_spaces <- c(location_data[,1], location_data[,2], location_data[,3])
...
# also outputs all locations landed on which idk how to get rid of
